...
import "./IShogunToken.sol";
import "./IShogunNFT.sol";
    ...
    ERC721HolderUpgradeable
{
    ...
    function startTraining(uint256[] memory shogunIds) public {
        ...
        for (uint256 i = 0; i < shogunIds.length; i++) {
            SS.safeTransferFrom(msg.sender, address(this), shogunIds[i]);
        }
    ...
    function endTraining(uint256 trainId) public {
        ...
        for (uint256 i = 0; i < families[trainId].shogunIds.length; i++) {
            SS.safeTransferFrom(address(this), msg.sender, families[trainId].shogunIds[i]);
        }
